...
VALIDATION_DIR = settings.get_setting(
    'Final State Selection', 'validation_dir')
...
def validate_dataset(version, dataset):
    ...
    for log_dir in progressbar.progressbar(process_log_dirs, redirect_stdout=True):
        ...
        if len(good_jobs) > 0:
            good_jobs_list = os.path.join(
                dataset_validation_dir, f'{process}_good.txt')
            with open(good_jobs_list, 'w') as f:
                for good_job in sorted(good_jobs):
                    f.write(f'{good_job}\n')
        if len(bad_jobs) > 0:
            bad_jobs_list = os.path.join(
                dataset_validation_dir, f'{process}_bad.txt')
...
def main():
    parser = argparse.ArgumentParser(
        description='Validate the fs ntuples for a particular dataset.')
