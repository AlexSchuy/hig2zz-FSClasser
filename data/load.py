...
def update_run_config(version, dataset, processes, feature_keys, combination_type, selection_type, selection_stage, model, run_dir):
    ...
    config['DATA'] = {'version': version, 'dataset': dataset, 'processes': utils.list_to_str(processes), 'feature_keys': utils.list_to_str(
        feature_keys), 'combination_type': combination_type, 'selection_type': str(selection_type), 'selection_stage': str(selection_stage), 'model': model, 'train_test_seed': str(train_test_seed)}
...
def get_selection(pids, version, dataset, combination_type='highest_pt', selection_type='standard', use_progressbar=True):
    ...
    if selection_type == 'ryuta':
        selection_features, _ = load_data(['InvisMass', 'MissingMass', 'RMass12', 'RMass34', 'Rreco34', 'nPFOs', 'VisPt',
                                           'MinA1Lab'], version, dataset, processes, combination_type='ryuta', use_progressbar=use_progressbar)
        # Cut 1: M(invisible) > M(jj)
        cuts['PassCut1'] = selection_features['InvisMass'] > selection_features['RMass12']
        # Cut 2: 80 GeV < M(mm) < 100 GeV
        cuts['PassCut2'] = (selection_features['RMass34'] > 80) & (
            selection_features['RMass34'] < 100)
        # Cut 3: 120 GeV < M_reco(mm) < 160 GeV
        cuts['PassCut3'] = (selection_features['Rreco34'] > 120) & (
            selection_features['Rreco34'] < 160)
        # Cut 4: nPFOs > 15
        ...
        cuts['PassCut7'] = (selection_features['InvisMass'] > 60) & (
            selection_features['RMass12'] < 45)
    elif selection_type == 'standard':
        selection_features, _ = load_data(['nPFOs', 'VisMass', 'CosTheta'], version, dataset,
                                          processes, combination_type=combination_type, use_progressbar=use_progressbar)
        # Cut 0: nPFOs >= 10
        cuts['PassCut0'] = (selection_features['nPFOs'] >= 10)
        # Cut 1: The visible mass should be in should be in [115, 130] GeV
        cuts['PassCut1'] = (selection_features['VisMass'] >= 115) & (
            selection_features['VisMass'] <= 130)
        # Cut 2: |Cos(theta)| < 0.9
...
def apply_selection(A, cuts, stage=-1):
    ...
    for i, cut in enumerate(cuts):
        ...
        logging.info('Cut {} eliminates {}%'.format(
            i, 100 * (1 - np.sum(cuts[cut] / cuts.shape[0]))))
...
def select_highest_pt_combinations(features, meta, version, dataset, use_progressbar=True):
    ...
    # Sort by fs pTs
    pts, _ = cache.load_data(['LeadingJetPt', 'SubLeadingJetPt', 'LeadingMuonPt', 'SubLeadingMuonPt',
...
def select_highest_E_combinations(features, meta, version, dataset, use_progressbar=True):
    ...
    # Sort by fs E
    E, _ = cache.load_data(['LeadingEnJetEn', 'SubLeadingEnJetEn', 'LeadingEnMuonEn',
...
def select_ryuta_combinations(features, meta, version, dataset='cross_check', use_progressbar=True):
    ...
    selection_features, selection_meta = cache.load_data(
        ['PfoEnP3', 'PfoEnP4', 'njets', 'RMass34', 'Event'], version, dataset, processes, use_progressbar=use_progressbar)

    # Select events with N(jet) = 2 and 10 GeV < E(m) < 100 GeV for m = m+, m-
    mask = (selection_features['njets'] == 2) & (selection_features['PfoEnP3'] > 10) & (
        selection_features['PfoEnP3'] < 100) & (selection_features['PfoEnP4'] > 10) & (selection_features['PfoEnP4'] < 100)
...
def load_data(feature_keys, version, dataset, processes, indices=None, combination_type=None, selection_type=None, selection_stage=-1, use_progressbar=True):
    ...
    assert combination_type in COMBINATION_TYPES or combination_type is None, 'combination_type={}'.format(
        combination_type)
    assert selection_type in SELECTION_TYPES or selection_type is None, 'selection_type={}'.format(
        selection_type)

    # Load data from cache.
    features, meta = cache.load_data(
        feature_keys, version, dataset, processes, use_progressbar=use_progressbar)
    ...
    if combination_type == 'ryuta':
        features, meta = select_ryuta_combinations(
            features, meta, version, dataset, use_progressbar=use_progressbar)
    elif combination_type == 'highest_pt':
        features, meta = select_highest_pt_combinations(
            features, meta, version, dataset, use_progressbar=use_progressbar)
    elif combination_type == 'highest_E':
        features, meta = select_highest_E_combinations(
            features, meta, version, dataset, use_progressbar=use_progressbar)
    ...
    if selection_type:
        cuts = get_selection(meta['pid'], version, dataset, selection_type=selection_type,
                             combination_type=combination_type, use_progressbar=use_progressbar)
        features, meta = apply_selection(
            [features, meta], cuts, selection_stage)
...
def load_train_test_data_from_run_dir(run_dir, data_type, get_indices=False, use_progressbar=True):
    ...
    if data_type not in ('train', 'test', 'both'):
        raise ValueError(
            'data_type must be in ("train", "test", "both") but was {}'.format(data_type))
    config = utils.load_run_config(run_dir)
    ...
    X, y = load_data(feature_keys, version, dataset, processes, combination_type=combination_type,
                     selection_type=selection_type, selection_stage=selection_stage, use_progressbar=use_progressbar)
    y = y['pid'].values
    skf = StratifiedKFold(
        n_splits=2, random_state=train_test_seed, shuffle=True)
